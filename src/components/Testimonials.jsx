      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <p className="text-sm sm:text-base text-slate-600 leading-relaxed">
            See how our structured curriculums and dedicated learning environment inspire students in their spiritual and intellectual growth.
          </p>
        </div>

        {/* Testimonials Framework Notice */}
        <div className="mt-8 max-w-2xl mx-auto p-4 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 text-xs flex items-center gap-3">
          <Info className="w-5 h-5 text-slate-400 shrink-0" />
          <p>
            <strong>Note:</strong> Design structure prepared for verified student reviews and feedback upon cohort completion.
          </p>
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {testimonialsData.map((item) => (
            <div
              key={item.id}
              className="bg-sand-50/70 rounded-2xl p-7 border border-slate-200/90 shadow-soft-card hover:shadow-card-hover transition-all duration-300 flex flex-col justify-between relative group"
            >
