...
import { staticSeoData } from '../data/seoData';
...
export default function NotFoundPage() {
  return (
    <div className="min-h-[75vh] flex items-center justify-center py-20 px-4 sm:px-6 lg:px-8 bg-sand-50">
      ...
      <SEO
        title={staticSeoData.notFound.title}
        description={staticSeoData.notFound.description}
        canonical={staticSeoData.notFound.canonical}
        noindex={true}
      />

      <div className="max-w-lg w-full text-center space-y-6 bg-white p-8 sm:p-12 rounded-3xl border border-slate-200 shadow-soft-card">
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-50 text-emerald-900 text-xs font-semibold uppercase tracking-wider border border-emerald-200">
          <IslamicStarDeco className="w-3.5 h-3.5 text-gold-600" />
          <span>Error 404</span>
        </div>
